  ...
  return (
    <section id="coding-profiles" className="relative py-24 sm:py-32 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 relative z-10">
        <SectionEyebrow command="git remote -v" title="Profiles" />

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-10" style={{ perspective: 1000 }}>
          {codingProfiles.map((p, i) => {
            ...
            return (
              <motion.div
                key={p.platform}
                initial={{ opacity: 0, scale: 0.85, rotate: i % 2 === 0 ? -3 : 3 }}
                whileInView={{ opacity: 1, scale: 1, rotate: 0 }}
                viewport={{ once: true, amount: 0.3 }}
                transition={{ duration: 0.55, delay: i * 0.08, ease: [0.34, 1.4, 0.64, 1] }}
              >
                <TiltCard maxTilt={6} className="h-full">
                  ...
                  >
                    ...
                    {/* Floating icon ghost */}
                    <motion.div
                      className={`pointer-events-none absolute -bottom-4 -right-4 text-7xl ${accent.icon} opacity-[0.08]`}
                      animate={{ y: [0, -8, 0] }}
