...
const PARTICLES = [
  { top: "15%", left: "10%" }, { top: "35%", left: "85%" },
  { top: "55%", left: "20%" }, { top: "70%", left: "70%" },
  { top: "25%", left: "55%" }, { top: "80%", left: "40%" },
];
      ...
      <div className="absolute inset-0 pointer-events-none opacity-[0.20]">
        {PARTICLES.map((pos, i) => (
          <motion.span
            ...
            style={pos}
            animate={{ y: [0, -24, 0], x: [0, 12, 0], opacity: [0.3, 1, 0.3] }}
            transition={{ duration: 9 + (i % 4), repeat: Infinity, ease: "easeInOut", delay: i * 0.6 }}
          />
        ))}
      </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {navLinks.map((link) => (
                <a
                  ...
                  className="group relative text-sm text-white/60 hover:text-gold-light transition-colors w-fit"
                >
                  {link.label}
                  <span className="absolute left-0 -bottom-0.5 h-px w-0 bg-gold-dark group-hover:w-full transition-all duration-300 ease-out" />
                </a>
              ))}
            </div>
          </div>
              ...
              {[
                ...
                return (
                  <motion.a
                    ...
                    whileHover={{ y: -3, backgroundColor: "#B8860B", color: "#FFFFFF" }}
