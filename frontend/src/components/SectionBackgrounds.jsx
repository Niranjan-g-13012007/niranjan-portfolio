...
export function RotatingGradientOrbs({ colors = ["var(--color-gold)", "var(--color-blue)"] }) {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      <motion.div
        className="absolute -top-24 -left-24 w-72 h-72 rounded-full blur-[100px] opacity-[0.10]"
        style={{ backgroundColor: colors[0] }}
        animate={{ scale: [1, 1.15, 1] }}
        transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute -bottom-24 -right-24 w-72 h-72 rounded-full blur-[100px] opacity-[0.10]"
